...
import { ProductPickerModal } from '../product/ProductPickerModal'

// Lazy-load preview modal so @react-pdf/renderer stays out of the main bundle
...
interface InvoiceFormProps {
  ...
  /** 'create' → addInvoice ; 'edit' → updateInvoice */
  mode: 'create' | 'edit'
  onSaveDraft: (invoice: Invoice) => void
  onEmit: (invoice: Invoice) => Promise<void>
}
...
export function InvoiceForm({
  ...
  const [client, setClient] = useState<Client>(
    initialValues?.client ?? EMPTY_CLIENT
  )
  const [seller, setSeller] = useState<Seller>(() => {
    if (initialValues?.seller) return initialValues.seller
    return {
      ...
      matriculeFiscal: settingsState.matriculeFiscal,
    }
  })
  const [items, setItems] = useState<LineItem[]>(
    initialValues?.items ?? [newLineItem()]
  )
  const [issueDate, setIssueDate] = useState(
    initialValues?.issueDate ?? new Date().toISOString().split('T')[0]
  )
  ...
  const fillItemFromProduct = (lineId: string, product: Product) => {
    setItems((prev) =>
      prev.map((item) => {
        if (item.id !== lineId) return item
        const calc = calculateLineItem(item.quantity, product.unitPrice, product.vatRate)
        return { ...item, description: product.name, unitPrice: product.unitPrice, vatRate: product.vatRate, ...calc }
      })
    )
  ...
  const validate = () => {
    ...
    if (!client.name.trim()) errs['client.name'] = t('errors.required')
    if (client.matriculeFiscal && !validateMatriculeFiscal(client.matriculeFiscal))
      errs['client.matriculeFiscal'] = t('errors.invalidMatricule')
    if (seller.matriculeFiscal && !validateMatriculeFiscal(seller.matriculeFiscal))
      errs['seller.matriculeFiscal'] = t('errors.invalidMatricule')
    if (!issueDate) errs['issueDate'] = t('errors.required')
    setErrors(errs)
    return Object.keys(errs).length === 0
  }

  const buildInvoice = (status: Invoice['status'] = 'draft'): Invoice => ({
    id: crypto.randomUUID(),
  ...
  const handlePreview = () => {
    if (!validate()) return
    setPreviewInvoice(buildInvoice('sent'))
  }

  const handleEmitFromModal = async () => {
    if (!previewInvoice) return
    setPreviewInvoice(null)
    await handleEmit(previewInvoice)
  }

  const handleDirectEmit = async () => {
    if (!validate()) return
    await handleEmit(buildInvoice('sent'))
  }

  const pageTitle = mode === 'edit' ? t('invoice.edit') : t('invoice.create')
    ...
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <section className="space-y-4 rounded-xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-gray-900">
          ...
          <Field label={t('invoice.client.select')} error={errors['client.name']} required>
              ...
              >
                ...
                {clients.map((c) => (
                  <option key={c.id} value={c.id}>
                    {c.name}
                    {c.matriculeFiscal ? ` — ${c.matriculeFiscal}` : ''}
                  </option>
                ))}
              </select>
              <button
          ...
          {client.name && (
            <div className="space-y-1 rounded-lg bg-gray-50 p-4 text-sm dark:bg-gray-800">
        ...
        <section className="space-y-4 rounded-xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-gray-900">
          ...
          <div className="grid grid-cols-2 gap-4">
            <Field label={t('invoice.issueDate')} error={errors['issueDate']} required>
              <input
                className={inputClass}
                type="date"
                value={issueDate}
          ...
          <Field label={t('invoice.sellerMatricule')} error={errors['seller.matriculeFiscal']}>
            <input
              className={inputClass}
                ...
                <tr key={item.id}>
                  <td className="px-4 py-2">
                    <div className="flex gap-1">
                      ...
                      <button
                        type="button"
                        onClick={() => setPickingProductForLine(item.id)}
                        className="flex-shrink-0 rounded-lg border border-gray-200 px-2 py-1.5 text-sm text-gray-500 hover:bg-gray-50 hover:text-primary-600 dark:border-gray-700 dark:hover:bg-gray-800"
                        title={t('invoice.lines.pickProduct')}
                      >
                        📦
                      </button>
                    </div>
                  ...
                  <td className="px-4 py-2 text-end font-medium text-gray-700 dark:text-gray-300" dir="ltr">
                    {formatCurrency(item.totalHT, language)}
                  </td>
                  <td className="px-4 py-2 text-end font-semibold text-gray-900 dark:text-white" dir="ltr">
      ...
      {pickingProductForLine && (
        <ProductPickerModal
          onSelect={(product) => fillItemFromProduct(pickingProductForLine, product)}
          onClose={() => setPickingProductForLine(null)}
        />
      )}

      {/* Preview modal — lazy-loaded so @react-pdf/renderer stays in a separate chunk */}
      {previewInvoice && (
